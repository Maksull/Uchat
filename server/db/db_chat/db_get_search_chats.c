...
cJSON *db_get_search_chats(const cJSON *chat_info, t_server_utils *utils)
{
    ...
    sqlite3_stmt *sql_stmt;                                                       // SQLite statement object
    ...
    sqlite3_bind_int64(sql_stmt, 2, utils->user->user_id);

    // Iterate over the search results and add each chat to the JSON array
    while (sqlite3_step(sql_stmt) == SQLITE_ROW)
    {
        cJSON_AddItemToArray(chats_json, sql_to_json_chat(sql_stmt, true)); // Convert current row to JSON and add to array
    }

...
static bool is_password_not_empty(const gchar *password)
{
    return mx_strcmp(password, "") != 0;
}

// Function to validate if the password length is within the specified range
static bool is_password_length_valid(const gchar *password)
{
...
static void handle_invalid_password(GtkWidget *password_field, GtkWidget *password_notify_label, const gchar *error_message)
{
...
{
    const gchar *user_password = gtk_entry_buffer_get_text(gtk_entry_get_buffer(GTK_ENTRY(password_field)));

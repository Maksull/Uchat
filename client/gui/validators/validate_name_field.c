...
static bool is_name_not_empty(const gchar *name)
{
    return mx_strcmp(name, "") != 0;
}

// Function to validate if the name length is within the specified range
static bool is_name_length_valid(const gchar *name)
{
...
static bool is_name_format_valid(const gchar *name)
{
    return is_user_name_format_valid(name);
}

// Function to set error style and notification for an invalid name
static void handle_invalid_name(GtkWidget *username_field, GtkWidget *username_notify_label, const gchar *error_message)
{
...
{
    const gchar *user_name = gtk_entry_buffer_get_text(gtk_entry_get_buffer(GTK_ENTRY(username_field)));

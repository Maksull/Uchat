...
// Function to create an event box for the chat item
...
// Function to create the chat list item box
static GtkWidget *create_chatlist_item_box(int id, GtkWidget *event_box)
{
...
// Function to add the chat ID as a hidden label
static void add_chat_id_label(int id, GtkWidget *chatlist_item)
{
    GtkWidget *chatlist_item_id = gtk_label_new(mx_itoa(id));
...
// Function to add the chat name as a label to the text block
static GtkWidget *add_chat_name_label(const char *chat_name, GtkWidget *chatlist_item_text)
{
...
void add_join_chat_item(int id, char *chat_name, t_avatar_color avatar_color)
{
    ...
    // Create the chat list item box
    GtkWidget *chatlist_item = create_chatlist_item_box(id, event_box);

    // Add the chat ID as a hidden label
    add_chat_id_label(id, chatlist_item);
    ...
    GtkWidget *chatlist_item_text = create_chatlist_item_text_block(chatlist_item);

    // Add the chat name as a label to the text block
    GtkWidget *chatlist_item_title = add_chat_name_label(chat_name, chatlist_item_text);

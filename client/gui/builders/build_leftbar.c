...
GtkWidget *create_new_chat_button(){
    ...
    g_signal_connect(G_OBJECT(new_chat_btn), "leave-notify-event", G_CALLBACK(on_crossing), NULL);
    g_signal_connect(G_OBJECT(new_chat_btn), "button_press_event", G_CALLBACK(popup_create_chat_menu), chat_screen);
    GtkWidget *create_chat_label = gtk_label_new("+ New chat");
    add_class(create_chat_label, "switch_auth_menu_label");
    gtk_container_add(GTK_CONTAINER(new_chat_btn), create_chat_label);
    return new_chat_btn;
}
...
void build_leftbar(GtkWidget *chat_screen)
{
    ...
    GtkWidget *new_chat_btn = create_new_chat_button();

    gtk_box_pack_start(GTK_BOX(header_block), header_label, FALSE, FALSE, 0);
    gtk_box_pack_end(GTK_BOX(header_block), new_chat_btn, FALSE, FALSE, 0);
    ...
    GtkWidget *clear_field_btn = create_clear_field_button();
    ...
    GtkWidget *edit_profile_btn = create_edit_profile_button();
